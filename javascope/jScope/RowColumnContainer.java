...
public class RowColumnContainer extends JComponent{
     ...
     * @see RowColumnLayout
     */
    public void update(final float ph[], final float pw[]) {
        this.ph = ph;
        this.pw = pw;
        this.update();
    }
     ...
     */
    public void update(final int rows[], final Component c[]) {
        if(rows == null || rows.length == 0) throw new IllegalArgumentException("Defined null or empty row column container");
        ...
        Btm b;
        int idx = 0;
        if(curr_rows.length > rows.length) col = curr_rows.length;
        else col = rows.length;
        for(int i = 0; i < col; i++){
            if(i > rows.length){
