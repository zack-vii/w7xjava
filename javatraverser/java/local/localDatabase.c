...
JNIEXPORT jobject JNICALL Java_local_localDatabase_getInfo
  ...
  constr =
      (*env)->GetStaticMethodID(env, cls, "getNodeInfo",
				"(BBBIIIIILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)LjTraverser/NodeInfo;");
    ...
    (JNIEnv * env, jobject obj, jstring jname, jint usage, jint context)
{
  ...
  cls = (*env)->FindClass(env, "mds.Data.NidData"");
...
JNIEXPORT jobjectArray JNICALL Java_local_localDatabase_startDelete
  ...
  jclass cls = (*env)->FindClass(env, "mds.Data.NidData");
  nid_fid = (*env)->GetFieldID(env, cls, "datum", "I");
...
JNIEXPORT jobject JNICALL Java_local_localDatabase_resolve
  ...
  cls = (*env)->FindClass(env, "mds.Data.NidData");
  constr = (*env)->GetStaticMethodID(env, cls, "getData", "(I)Lmds/Data/Data;");
  args[0].i = nid;
  return (*env)->CallStaticObjectMethodA(env, cls, constr, args);
}
...
//      printf("value = %d error %d \n", *(int *)out->pointer, errno);
  ...
  case DTYPE_METHOD:
	...
	if (status & 1)
      ...
      {
	if (status & 1)
	  status = TreeDoMethod(&nid_d, method_d_ptr->method, method_d_ptr->arguments[0], method_d_ptr->arguments[1], &retStatus_d MDS_END_ARG);
      }
      if (status & 1)
	status = retStatus;
    } else
